...
# start_date = (date.today() - MonthBegin(3)).date()
first_period = pd.to_datetime(start_date).to_period('M')
...
escala_colores_porciento = [{'range': [idx, idx + .5 / escala_len], 'color': color} for idx, color in
                            zip(np.arange(0.5, 1.01, 0.5 / escala_len), px.colors.diverging.RdYlGn[:-1])]
...
def fetch_kpi_data():
    ...
    kpi_contratos_pre = contratos.loc[first_period].to_dict()
    try:
        kpi_contratos = contratos.iloc[1].to_dict()
    except IndexError:
        kpi_contratos = {'total_ra': 0,
                         'total_sib': 0,
                         'RA': 0,
                         'SIB': 0,
                         'A tiempo': 0,
...
def render_content(tab, kpi_data):
    if tab == 'tab-1':
        ...
        return html.Div([
            ...
            # ]),
            # dcc.Graph(figure=create_gauge_chart(title_casos, kpi_casos, kpi_casos_prev, tipo_grafico='cases'))
            dcc.Graph(figure=create_gauge_chart(title_dias_ra, kpi_dias_ra, kpi_dias_prev_ra, tipo_grafico='days',
                                                width=650)),
            dcc.Graph(figure=create_gauge_chart(title_dias_sib, kpi_dias_sib, kpi_dias_prev_sib, tipo_grafico='days',
                                                width=650)),
            dcc.Graph(
                figure=create_gauge_chart(title_casos, kpi_casos, kpi_casos_prev, tipo_grafico='cases', width=650))
            # ], style={'width': '50%', 'display': 'flex', 'flex-direction': 'column', 'margin': 'auto'}),
...
def update_title_mes(n_intervals):
    fecha = date.today()
    # fecha = (date.today() - MonthBegin(2)).date()
    if fecha.day < 3:
...
if __name__ == '__main__':
    ...
    # context = ("..\\credentials\\certificado_sb.crt")
    serve(app.server, host=host, port=port)

    # ssl_context = ("C:\\Users\\jacevedo\\OneDrive - Superintendencia de Bancos de la Republica Dominicana\\Documents\\ProUsuario Project\\credentials\\cert.pem",
    #                "C:\\Users\\jacevedo\\OneDrive - Superintendencia de Bancos de la Republica Dominicana\\Documents\\ProUsuario Project\\credentials\\key.pem")
    # app.run_server(ssl_context=ssl_context, host=host, port=port)
    # app.run_server(host=host, port=port, ssl_context=('./ssl_cert.crt', './ssl_cert.key'))
